...
document.addEventListener('keydown', (event) => {
    ...
    if (key === 'ArrowDown') {
        ...
        if (active === sInput) {
            setActiveResult(firstResult.querySelector('.entry-title-link'));
        } else if (active?.parentElement !== lastResult) {
            setActiveResult(active?.parentElement?.nextElementSibling?.querySelector('.entry-title-link'));
        }
    } else if (key === 'ArrowUp') {
        event.preventDefault();
        ...
        if (active?.parentElement === firstResult) {
            ...
            setActiveResult(active?.parentElement?.previousElementSibling?.querySelector('.entry-title-link'));
        }
    } else if (key === 'ArrowRight') {
        if (active?.matches?.('.entry-title-link')) {
            active.click();
        }
    }
});
